      ...
      function onFrame() {
        ...
        function moutainHeight(position, rough) {
          ...
          return frequencies.reduce(
            (height, freq) => height * rough - Math.cos(freq * position),
            0
          );
        }
        ...
        for (let i = 0; i < 4; i++) {
          ...
          for (let x = canvas.width; x--; ) {
            ...
            let y =
              128 + i * 25 + moutainHeight(montainPosition, moutainRough) * 25; // 振幅
            cxt.fillRect(x, y, 1, 999);
            godraysCxt.fillRect(x / 4, y / 4 + 1, 1, 999);
          }
        }
        cxt.globalCompositeOperation = godraysCxt.globalCompositeOperation =
          "lighter";
